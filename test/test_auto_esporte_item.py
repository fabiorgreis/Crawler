"""Test AutoEsporteItem Class Module"""
from bs4 import BeautifulSoup
from crawler.auto_esporte_item import AutoEsporteItem

def test_processa_item():
    """Testa o processamento do item com o exemplo dado no github"""
    item = """<item>
...
<p>
    ...
    proc_item = AutoEsporteItem()
    data = proc_item.processa_item(soup)
    assert data['title'] == 'Volkswagen Arteon aparece às vésperas do Salão de Genebra'
    assert data['link'] == 'http://revistaautoesporte.globo.com/Noticias/noticia/2017/03/'\
                           'volkswagen-arteon-aparece-vesperas-do-salao-de-genebra.html'
    assert data['content'][0]['content'] == 'http://s2.glbimg.com/DIgFxpN0aAu99uvuZ3WmSqAU'\
                                            'V2E=/620x413/e.glbimg.com/og/ed/f/original/2017'\
                                            '/03/06/1_WKIn5xh.jpg'
    assert data['content'][0]['type'] == 'image'
    assert data['content'][-3]['type'] == 'text'
    assert data['content'][-2]['type'] == 'links'
    assert len(data['content'][-2]['content']) == 2
